 ...
 * stesso pezzo di mondo nello stesso momento. Il battito del minuto e la
 * richiesta del giocatore sono processi diversi — il flock di bin/tick.php
 * protegge solo i battiti fra loro — e senza lucchetto possono partire dallo
 * stesso istante simulato, rifare la stessa finestra di tempo e applicarne due
 * volte gli effetti: righe di diario doppie, incontri aperti due volte, tiri
 * ripetuti.
 *
 * Si usa GET_LOCK di MariaDB perche' vive nel database, che e' l'unica cosa
 * che tutti i processi vedono di sicuro. Il nome porta il prefisso del
 * database perche' i lucchetti sono globali al server e qui sopra girano altri
 * siti.
 */
final class Lock
{
    ...
    private static function nomeCompleto(string $nome): string
    {
        ...
        return substr('kor:' . (string) Config::get('db.name', 'orangeroad') . ':' . $nome, 0, 64);
    }
